...
function renderTable() {
  ...
  tbody.innerHTML = sorted.map(r => `
    ...
    <tr class="delete-action-row" data-for="${r.id}" style="display:none">
      <td colspan="4">
        <div style="display:flex;gap:8px;padding:8px 12px;background:rgba(239,68,68,0.08)">
          <button class="del-one" data-id="${r.id}" style="font-size:13px;color:#EF4444;background:none;border:none;cursor:pointer;padding:4px 8px">Excluir esta</button>
          ...
          <button class="del-cancel" data-id="${r.id}" style="font-size:13px;color:var(--text-2);background:none;border:none;cursor:pointer;padding:4px 8px;margin-left:auto">Cancelar</button>
        </div>
      </td>
    </tr>
  `).join('');

  // row tap -> show delete actions
  tbody.querySelectorAll('.tx-row').forEach(row => {
    row.addEventListener('click', () => {
      const id = row.dataset.id;
      const actionRow = tbody.querySelector(`.delete-action-row[data-for="${id}"]`);
      if (actionRow.style.display === 'none') {
        tbody.querySelectorAll('.delete-action-row').forEach(r=>r.style.display='none');
  ...
  tbody.querySelectorAll('.del-cancel').forEach(btn => btn.addEventListener('click', e => {
    e.stopPropagation();
    tbody.querySelector(`.delete-action-row[data-for="${btn.dataset.id}"]`).style.display='none';
  }));
}
